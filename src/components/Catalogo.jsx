...
import { Search, Star, Heart, ShoppingBag, X, MessageCircle, ChevronRight } from 'lucide-react'
...
export default function Catalogo() {
  ...
  const { ref: decoRef, offset: decoOffset } = useParallax(0.1)
  ...
  return (
    <section id="catalogo" aria-label="Catalogo completo" className="relative py-16 sm:py-24 bg-white overflow-hidden">
      {/* Parallax decorative elements */}
      <div ref={decoRef} className="absolute inset-0 pointer-events-none hidden md:block" aria-hidden="true">
        <div
          className="absolute -top-24 right-[10%] w-72 h-72 bg-pink-100/30 rounded-full blur-3xl will-change-transform"
          style={{ transform: `translateY(${decoOffset * 1.5}px)` }}
        />
        <div
          className="absolute bottom-20 -left-16 w-60 h-60 bg-rose-50/40 rounded-full blur-3xl will-change-transform"
          style={{ transform: `translateY(${decoOffset * -1.2}px)` }}
        />
        <div
          className="absolute top-1/2 left-[5%] w-20 h-20 border-2 border-pink-100/40 rounded-full will-change-transform"
          style={{ transform: `translateY(${decoOffset * 2.5}px) rotate(${decoOffset * 0.3}deg)` }}
        />
      </div>
      ...
      <div ref={sectionRef} className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`text-center mb-8 sm:mb-12 transition-all duration-700 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <span className="text-pink-600 font-semibold text-xs sm:text-sm tracking-wider uppercase">Catalogo Completo</span>
          ...
          <p className="text-gray-500 max-w-2xl mx-auto text-sm sm:text-lg px-2">
            Encuentra el postre perfecto para cada ocasion. Haz click en cualquier producto para ver mas detalles.
          </p>
        </div>

        {/* Search + Filters */}
        <div className={`mb-8 sm:mb-10 transition-all duration-700 delay-200 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          {/* Search bar */}
          <div className="relative max-w-md mx-auto mb-6">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 rounded-full border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent transition-all text-sm"
            />
            {busqueda && (
              <button
                onClick={() => setBusqueda('')}
                aria-label="Limpiar busqueda"
                className="absolute right-3 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-gray-200 hover:bg-gray-300 flex items-center justify-center transition-colors"
              >
          ...
          {/* Category filters */}
          <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-2 sm:pb-0 sm:flex-wrap sm:justify-center scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
            {categorias.map(cat => (
              <button
                key={cat}
            ...
            <Card3D key={producto.id}>
              ...
              >
                ...
                <div className="relative overflow-hidden aspect-square">
                  ...
                  <div className="absolute bottom-2 sm:bottom-3 left-2 sm:left-3 right-2 sm:right-3 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 hidden sm:block">
                    <div className="bg-white/90 backdrop-blur-sm rounded-lg px-3 py-2 text-center">
                      <span className="text-xs font-medium text-gray-700">Click para ver detalle</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 sm:p-4">
                  ...
                  <p className="text-xs text-gray-400 mb-2 line-clamp-1 hidden sm:block">{producto.descripcion}</p>
